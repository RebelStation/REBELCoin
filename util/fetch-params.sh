...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/REBELCoinParams"
    else
        PARAMS_DIR="$HOME/.rebelcoin-params"
    fi
fi
...
Failed to fetch the REBELCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
REBELCoin - fetch-params.sh

This script will fetch the REBELCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common REBELCoin zkSNARK parameters. Note that it is
