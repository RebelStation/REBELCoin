...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("REBELCoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("REBELCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("rebelcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running REBELCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("REBELCoinMiner exception");
    } catch (...) {
        LogPrintf("REBELCoinMiner exception");
    }

    LogPrintf("REBELCoinMiner exiting\n");
}
