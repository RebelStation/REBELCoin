...
const unsigned char REBELCoin_HD_SEED_FP_PERSONAL[crypto_generichash_blake2b_PERSONALBYTES] =
    {'P', 'I', 'V', 'X', '_', '_', 'H', 'D', '_', 'S', 'e', 'e', 'd', '_', 'F', 'P'};

const unsigned char REBELCoin_TADDR_OVK_PERSONAL[crypto_generichash_blake2b_PERSONALBYTES] =
...
{
    CBLAKE2bWriter h(SER_GETHASH, 0, REBELCoin_HD_SEED_FP_PERSONAL);
    h << seed;
    return h.GetHash();
}
...
uint256 ovkForShieldingFromTaddr(HDSeed& seed) {
    ...
    assert(crypto_generichash_blake2b_init_salt_personal(
            ...
            64,
            NULL,    // No salt.
            REBELCoin_TADDR_OVK_PERSONAL) == 0);
