...
static UniValue voteProposal(const uint256& propHash, const CBudgetVote::VoteDirection& nVote, const mnKeyList& mnKeys, UniValue resultsObj, int failed)
{
    int success = 0;
    for (const auto& k : mnKeys) {
		...
		if (!g_budgetman.ProcessProposalVote(vote, nullptr)) {
			resultsObj.push_back(packErrorRetStatus(k.mnAlias, "Failed to process"));
            failed++;
            continue;
        }
    ...
    int failed)
{
    ...
    for (const auto& k : mnKeys) {
		...
		if (!g_budgetman.ProcessFinalizedBudgetVote(vote, nullptr)) {
...
UniValue mnBudgetVoteInner(CWallet* const pwallet, bool fLegacyMN, const uint256& budgetHash, bool fFinal, const CBudgetVote::VoteDirection& nVote, const Optional<std::string>& mnAliasFilter)
{
    ...
    return (fFinal ? voteFinalBudget(budgetHash, mnKeys, resultsObj, failed) : voteProposal(budgetHash, nVote, mnKeys, resultsObj, failed));
}
...
UniValue mnLocalBudgetVoteInner(bool fLegacyMN, const uint256& budgetHash, bool fFinal, const CBudgetVote::VoteDirection& nVote)
{
    ...
    return (fFinal ? voteFinalBudget(budgetHash, mnKeys, resultsObj, 0) : voteProposal(budgetHash, nVote, mnKeys, resultsObj, 0));
}