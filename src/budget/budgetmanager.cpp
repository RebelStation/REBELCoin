...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if (nHeight > 0 && nHeight < nLast) {
        return 1 * COIN;
    } else if (nHeight > nLast && nHeight <= 2664200) {
        return 4000 * COIN;
    } else if (nHeight > 2664200 && nHeight <= 5328200) {
        return 3000 * COIN;
    } else if (nHeight > 5328200 && nHeight <= 7992200) {
        return 2000 * COIN;
    } else {
        return 1000 * COIN;
    }
...
bool CheckCollateral(const uint256& nTxCollateralHash, const uint256& nExpectedHash, std::string& strError, int64_t& nTime, int nCurrentHeight, bool fBudgetFinalization)
{
    ...
    for (const CTxOut& o : txCollateral->vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 RBL finalization collateral
            //       will also cover the old 50 RBL finalization collateral.
