    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            if (randiter % 20 < 2 || coinbase_coins.size() < 10) {
                // REBELCoin: don't test for duplicate coinbases as those are not possible due to
...
BOOST_AUTO_TEST_CASE(ccoins_serialization)
{
    ...
    // REBELCoin: Example with fCoinStake
...
BOOST_AUTO_TEST_CASE(ccoins_add)
{
     ...
     *
     * REBELCoin: Remove Coinbase argument (ref: https://github.com/REBELCoin-Project/REBELCoin/pull/1775)
     *
