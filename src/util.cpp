...
const char * const REBELCoin_CONF_FILENAME = "rebelcoin.conf";
const char * const REBELCoin_PID_FILENAME = "rebelcoin.pid";
const char * const REBELCoin_MASTERNODE_CONF_FILENAME = "masternode.conf";


// REBELCoin only features
// Masternode
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\REBELCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\REBELCoinParams
    // Mac: ~/Library/Application Support/REBELCoinParams
    // Unix: ~/.rebelcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "REBELCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "REBELCoinParams";
#else
    // Unix
    return pathRet / ".rebelcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/rebelcoin/sapling-spend.params";
        sapling_output = "/usr/share/rebelcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/rebelcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/rebelcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", REBELCoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", REBELCoin_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty rebelcoin.conf if it does not exist
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", REBELCoin_PID_FILENAME));
