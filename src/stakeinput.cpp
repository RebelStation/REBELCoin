...
CRblStake* CRblStake::NewRblStake(const CTxIn& txin)
{
    ...
    return new CRblStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CRblStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CRblStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CRblStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CRblStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CRblStake::GetUniqueness() const
{
    //The unique identifier for a RBL stake is the outpoint
...
const CBlockIndex* CRblStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CRblStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CRblStake::ContextCheck(int nHeight, uint32_t nTime)
{
