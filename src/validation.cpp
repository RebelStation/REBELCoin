...
#if defined(NDEBUG)
#error "REBELCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in urbl) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetTreasuryValue(int nHeight) {
	int64_t nAmount = 0;
	
	if (nHeight > Params().GetConsensus().nTreasuryStartHeight) {
		  nAmount = (GetBlockValue(nHeight)) * 0.10;  // 10% Treasury Reward
	}
...
CAmount GetBlockValue(int nHeight)
{
   ...
   } else if (nHeight >= nLast && nHeight <= 266600) {
        return 2 * COIN;
    } 
    else if (nHeight > 266600 && nHeight <= 533000) {
        return 1.84 * COIN;
    }
    else if (nHeight > 533000 && nHeight <= 799400) {
        return 1.693 * COIN;
    }
    else if (nHeight > 799400 && nHeight <= 1065800) {
        return 1.557 * COIN;
    }
    else if (nHeight > 1065800 && nHeight <= 1332200) {
        return 1.433 * COIN;
    }
    else if (nHeight > 1332200 && nHeight <= 1598600) {
        return 1.318 * COIN;
    }
    else if (nHeight > 1598600 && nHeight <= 1865000) {
        return 1.213 * COIN;
    }
    else if (nHeight > 1865000 && nHeight <= 2131400) {
        return 1.116 * COIN;
    }
    else if (nHeight > 2131400 && nHeight <= 2397800) {
        return 1.026 * COIN;
    }
    else if (nHeight > 2397800 && nHeight <= 2664200) {
        return 0.944 * COIN;
    }
    else if (nHeight > 2664200 && nHeight <= 2930600) {
        return 0.869 * COIN;
    }
    else if (nHeight > 2930600 && nHeight <= 3197000) {
        return 0.799 * COIN;
    }
    else if (nHeight > 3197000 && nHeight <= 3463400) {
        return 0.735 * COIN;
    }
    else if (nHeight > 3463400 && nHeight <= 3729800) {
        return 0.677 * COIN;
    }
    else if (nHeight > 3729800 && nHeight <= 3996200) {
        return 0.622 * COIN;
    }
    else if (nHeight > 3996200 && nHeight <= 4262600) {
        return 0.573 * COIN;
    }
    else if (nHeight > 4262600 && nHeight <= 4529000) {
        return 0.527 * COIN;
    }
    else if (nHeight > 4529000 && nHeight <= 4795400) {
        return 0.485 * COIN;
    }
    else if (nHeight > 4795400 && nHeight <= 5061800) {
        return 0.446 * COIN;
    }
    else if (nHeight > 5061800 && nHeight <= 5328200) {
        return 0.410 * COIN;
    }
    else if (nHeight > 5328200 && nHeight <= 5594600) {
        return 0.377 * COIN;
    }
    else if (nHeight > 5594600 && nHeight <= 5861000) {
        return 0.347 * COIN;
    }
    else if (nHeight > 5861000 && nHeight <= 6127400) {
        return 0.319 * COIN;
    }
    else if (nHeight > 6127400 && nHeight <= 6393800) {
        return 0.294 * COIN;
    }
    else if (nHeight > 6393800 && nHeight <= 6660200) {
        return 0.270 * COIN;
    }
    else if (nHeight > 6660200 && nHeight <= 6926600) {
        return 0.249 * COIN;
    }
    else if (nHeight > 6926600 && nHeight <= 7193000) {
        return 0.229 * COIN;
    }
    else if (nHeight > 7193000 && nHeight <= 7459400) {
        return 0.211 * COIN;
    }
    else if (nHeight > 7459400 && nHeight <= 7725800) {
        return 0.194 * COIN;
    }
    else if (nHeight > 7725800 && nHeight <= 7992200) {
        return 0.178 * COIN;
    }
    else if (nHeight > 7992200 && nHeight <= 8258600) {
        return 0.164 * COIN;
    }
    else if (nHeight > 8258600 && nHeight <= 8525000) {
        return 0.151 * COIN;
    }
    else if (nHeight > 8525000 && nHeight <= 8791400) {
        return 0.139 * COIN;
    }
    else if (nHeight > 8791400 && nHeight <= 9057800) {
        return 0.128 * COIN;
    }
    else if (nHeight > 9057800 && nHeight <= 9324200) {
        return 0.117 * COIN;
    }
    else if (nHeight > 9324200 && nHeight <= 9590600) {
        return 0.108 * COIN;
    }
    else if (nHeight > 9590600 && nHeight <= 9857000) {
        return 0.099 * COIN;
    }
    else if (nHeight > 9857000 && nHeight <= 10123400) {
        return 0.091 * COIN;
    }
    else if (nHeight > 10123400 && nHeight <= 10389800) {
        return 0.084 * COIN;
    }
    else if (nHeight > 10389800 && nHeight <= 10656200) {
        return 0.077 * COIN;
    }
    else {
        return 0.071 * COIN;
...
{
    util::ThreadRename("rebelcoin-scriptch");
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
		...
		CAmount treasuryValue = GetTreasuryValue(nHeight);

        if (nHeight > Params().GetConsensus().nTreasuryStartHeight) {
			if (secondLastOut.nValue == masternodePayment && lastOut.nValue == treasuryValue)
				return true;
		}
		else {
	...
	if (nHeight > Params().GetConsensus().nTreasuryStartHeight) {
		...
		if (treasuryValue > 0) {
			...
			for (const CTxOut &txout : block.vtx[1]->vout) {
				...
				if ((found = EncodeDestination(Dest) == Params().GetConsensus().strTreasuryAddr && txout.nValue == treasuryValue) == true)
					break;
			}
			if (!found)
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> rblInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                rblInputs.push_back(stakeIn);
        }
        const bool hasRBLInputs = !rblInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasRBLInputs) {
                    ...
                    for (const CTxIn& rblIn : rblInputs)
                        if(rblIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasRBLInputs) {
                            for (const CTxIn& stakeIn : rblInputs)
