...
#include "qt/rebelcoin/settings/settingsfaqwidget.h"
#include "qt/rebelcoin/qtutils.h"
#include "qt/rebelcoin/settings/forms/ui_settingsfaqwidget.h"
#include <QMetaObject>
#include <QScrollBar>

SettingsFaqWidget::SettingsFaqWidget(REBELCoinGUI* parent) : QDialog(parent),
                                                                           ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({ui->labelSubtitle_Intro,
                       ui->labelSubtitle_UnspendableRBL,
    ...
    setCssProperty({ui->labelContent_Intro,
                       ui->labelContent_UnspendableRBL,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("REBEL Coin is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. REBEL Coin incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("REBEL Coin utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of REBEL Coin to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableRBLContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received REBEL Coin requires 6 confirmations on the network "
               "to become eligible for spending which can take ~12-15 minutes.")) +
        formatFAQParagraph(
            tr("Your REBEL Coin wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableRBL->setText(unspendableRBLContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of REBEL Coin with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("rebel coin", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("You can reach us by Email :  technical@rebelstation.org Tell us everything you can about the issue and we will get you the right support.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node REBEL Coin wallet with a "
               "requirement of 1000 RBL secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in REBEL Coin Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the REBEL Coin network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("1000 RBL per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 1000 RBL collateral "
    ...
    connect(ui->pushButton_UnspendableRBL, &QPushButton::clicked, [this]() { onFaqClicked(ui->widget_UnspendableRBL); });
    ...
    if (parent)
        connect(parent, &REBELCoinGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
