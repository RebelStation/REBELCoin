...
#include "qt/rebelcoin/forms/ui_snackbar.h"
#include "qt/rebelcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(REBELCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &REBELCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
