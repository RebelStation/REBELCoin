...
#include "qt/rebelcoin/forms/ui_masternodeswidget.h"

#include "qt/rebelcoin/masternodewizarddialog.h"
#include "qt/rebelcoin/mninfodialog.h"
#include "qt/rebelcoin/mnrow.h"
...
MasterNodesWidget::MasterNodesWidget(REBELCoinGUI* parent) : PWidget(parent),
    ...
    if (dialog->exportMN) {
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the rebelcoin.conf file\nof your remote server and start it, "
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                       ...
                       "# Example: mn1 127.0.0.2:18150 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= (COIN * 25000)) {
        inform(tr("Not enough balance to create a masternode, 1000 %1 required.").arg(CURRENCY_UNIT.c_str()));
        return;
    }
    showHideOp(true);
