...
#include "qt/rebelcoin/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/rebelcoin/forms/ui_dashboardwidget.h"
#include "qt/rebelcoin/qtutils.h"
#include "qt/rebelcoin/sendconfirmdialog.h"
#include "qt/rebelcoin/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(REBELCoinGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareRbl, "square-chart-rbl");
    setCssProperty(ui->labelRbl, "text-chart-rbl");
    ...
    setCssProperty(ui->labelAmountRbl, "text-stake-rbl-disable");
    ...
    if (window)
        connect(window, &REBELCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->btnHowTo, &QPushButton::clicked, [this]() { window->openFAQURL(); });
    ...
    // update the display unit, to not use the default ("RBL")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair RBL
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            rbl = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalRbl += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesRbl.append(rbl);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(249, 163, 63));
    set1->setColor(QColor(249, 163, 63));
    ...
    set0->append(chartData->valuesRbl);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalRbl > 0) {
        setCssProperty(ui->labelAmountRbl, "text-stake-rbl");
    } else {
        setCssProperty(ui->labelAmountRbl, "text-stake-rbl-disable");
    }
    forceUpdateStyle({ui->labelAmountRbl});
    ui->labelAmountRbl->setText(GUIUtil::formatBalance(chartData->totalRbl, nDisplayUnit));
