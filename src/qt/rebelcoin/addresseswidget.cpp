...
#include "qt/rebelcoin/forms/ui_addresseswidget.h"
#include "qt/rebelcoin/addresslabelrow.h"
#include "qt/rebelcoin/addnewaddressdialog.h"
#include "qt/rebelcoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(REBELCoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto rblAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(rblAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(rblAdd);
        if (walletModel->updateAddressBookLabels(rblAdd, label.toUtf8().constData(),
