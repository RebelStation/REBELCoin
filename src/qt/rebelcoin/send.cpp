...
#include "qt/rebelcoin/send.h"
#include "qt/rebelcoin/forms/ui_send.h"
#include "qt/rebelcoin/addnewcontactdialog.h"
#include "qt/rebelcoin/qtutils.h"
#include "qt/rebelcoin/sendchangeaddressdialog.h"
#include "qt/rebelcoin/optionbutton.h"
#include "qt/rebelcoin/sendconfirmdialog.h"
#include "qt/rebelcoin/guitransactionsutils.h"
#include "qt/rebelcoin/loadingdialog.h"
...
SendWidget::SendWidget(REBELCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    //setCssProperty(coinIcon, "coin-icon-rbl"); remove icon from send widget
    setCssProperty(coinIcon, "");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onRBLSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onRBLSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent RBLs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield RBLs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent RBLs to shield."));
    }
}
...
void SendWidget::onRBLSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto rblAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(rblAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(rblAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(rblAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
