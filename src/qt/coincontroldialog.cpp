...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select RBL Outputs to Spend" :
            "Select Shielded RBL to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::RBL;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show REBELCoin address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
