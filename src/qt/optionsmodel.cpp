...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::RBL);
    ...
    if (settings.contains("nAnonymizerebelcoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizerebelcoinamount", settings.value("nAnonymizerebelcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rebelcoin.cpp during shotdown to also remove the window positions
