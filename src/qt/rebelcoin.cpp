...
#include "config/rebelcoin-config.h"
#endif
...
#include "qt/rebelcoin/splash.h"
#include "qt/rebelcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("rebel coin", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in rebelcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in rebelcoin.qrc)
...
/** Class encapsulating REBEL Coin startup and shutdown.
...
/** Main REBELCoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (REBELCoinGUI)
...
{
    window = new REBELCoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &REBELCoinGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &REBELCoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(REBELCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(REBELCoinGUI::DEFAULT_WALLET);
        ...
        // REBELCoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &REBELCoinGUI::handlePaymentRequest);
        connect(window, &REBELCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. REBEL Coin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(rebelcoin_locale);
    ...
    /// 6. Determine availability of data directory and parse rebelcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("REBEL Coin"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("REBEL Coin"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // rebelcoin: links repeatedly have their payment requests routed to this process:
