...
/**
 * Fees smaller than this (in urbl) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != MN_COLL_AMT) {
        strError = "Invalid collateral tx value, must be 1000 RBL";
        ...
        std::vector<CStakeableOutput>* availableCoins)
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CRblStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
		...
		if (pindexPrev->nHeight + 1 > Params().GetConsensus().nTreasuryStartHeight) {
			nCredit += GetBlockValue(pindexPrev->nHeight + 1) - GetTreasuryValue(pindexPrev->nHeight + 1);
		}
		else {
		...
		if (pindexPrev->nHeight + 1 > Params().GetConsensus().nTreasuryStartHeight) {
			...
			CAmount treasuryValue = GetTreasuryValue(pindexPrev->nHeight + 1);

			CTxDestination treasuryAddress = DecodeDestination(Params().GetConsensus().strTreasuryAddr);
			CScript payee = GetScriptForDestination(treasuryAddress);

			int ops = (int)txNew.vout.size();
			txNew.vout.resize(ops + 1);
			txNew.vout[ops].scriptPubKey = payee;
			txNew.vout[ops].nValue = treasuryValue;
		}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in RBL) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of REBEL Coin"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart REBEL Coin to complete"));
            return nullptr;
        } else {
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_REBELCoin && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_REBELCoin);
        }
