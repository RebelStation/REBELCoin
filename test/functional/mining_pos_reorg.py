...
class ReorgStakeTest(rebelcoinTestFramework):
    ...
    def check_money_supply(self, expected_rbl):
        # verify that nodes have the expected RBL supply
        rbl_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(rbl_supply, [DecimalAmt(expected_rbl)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that RBL supply was properly updated after the reorgs
        self.log.info("Check RBL supply...")
