...
The build script we'll be using is contained in the REBELCoin github repository ([contrib/gitian-build.py](https://github.com/RebelStation/REBELCoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the REBELCoin repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/RebelStation/REBELCoin/master/contrib/gitian-build.py
